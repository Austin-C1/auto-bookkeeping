...
class CorsPreflightFilterTest {
    ...
    fun `api options request is handled before controller routing`() {
        val filter = CorsPreflightFilter("http://127.0.0.1:18882,http://localhost:18882")
        val request = MockHttpServletRequest("OPTIONS", "/api/auth/local-login").apply {
            addHeader("Origin", "http://127.0.0.1:18882")
        ...
        assertEquals("http://127.0.0.1:18882", response.getHeader("Access-Control-Allow-Origin"))
    ...
    fun `non options api request continues to controller routing`() {
        val filter = CorsPreflightFilter("http://127.0.0.1:18882")
