...
这份说明给第一次安装的人使用，重点包含 Docker Desktop 的下载、安装、配置，以及自动做账的启动、更新、快捷方式和数据保留。
...
https://github.com/Austin-C1/auto-bookkeeping/releases
...
第一次安装不要下载 update 包。update 包只给已经装过的人升级用。

## 七、第一次安装自动做账

步骤如下：

1. 把 `AutoBookkeeping-Blank-v1.0.8.zip` 解压出来。
2. 建议放在固定目录，例如：

```text
C:\AutoBookkeeping-Blank-v1.0.8
```

3. 不要直接在压缩包里面双击运行。
4. 不建议放在太深的目录，也不建议放在权限受限的系统目录。
5. 打开 Docker Desktop，并等待 Docker Desktop 运行完成。
6. 双击解压目录里的 `launch-blackcat.cmd`。
7. 第一次启动会自动创建本机数据库容器。
8. 启动完成后，浏览器会自动打开：
...
如果桌面已有旧快捷方式，程序会把它改成当前版本的启动路径。出现打不开时，先右键快捷方式查看目标位置，目标应该指向当前解压目录里的：
...
| 文件 | 用途 |
|---|---|
| `launch-blackcat.cmd` | 启动后端、前端和数据库，并打开做账页面 |
| `open-blackcat-frontend.cmd` | 只重新打开前端页面 |
| `README.txt` | 简短说明 |
| `01-install-docker-desktop.md` | Docker 安装说明 |
| `02-start-blackcat-blank.md` | 启动和使用说明 |

正常使用只需要双击 `launch-blackcat.cmd` 或桌面的 `自动做账启动`。

## 十、安装包使用的数据

空白安装包会创建独立的 Docker 数据库：

```text
auto-bookkeeping-empty-v1.0.8-mysql
```
...
如果远程更新失败，再去 GitHub Releases 下载完整安装包或更新包。
...
2. 启动窗口是否有报错。
3. 是否有其他程序占用 `18880` 或 `18001`。
4. 重新双击 `launch-blackcat.cmd`。
5. 浏览器按 `Ctrl + F5` 强制刷新。

### 7. Windows 提示脚本不安全

这是 Windows 对 `.cmd` 和 `.ps1` 文件的提示。
...
1. 打开 Docker Desktop。
2. 找到容器 `auto-bookkeeping-empty-v1.0.8-mysql`。
3. 停止并删除容器。
4. 删除对应的数据卷。
